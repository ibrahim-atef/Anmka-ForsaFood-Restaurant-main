...
class FireStoreUtils {
  ...
  static Future<List<VendorCategoryModel>> getHomeVendorCategory() async {
    ...
    await fireStore
        ...
        .where("show_in_homepage", isEqualTo: false)
...
static Future<UserModel?> getUserProfile(String uuid) async {
  print('🔍 Starting getUserProfile for UUID: $uuid');
  ...
  try {
    ...
    print('📄 Firestore document fetched: ${docSnapshot.exists ? "Exists" : "Does not exist"}');

    if (docSnapshot.exists) {
      final data = docSnapshot.data();
      print('✅ User data for $uuid: $data');
  ...
  print('🔚 Finished getUserProfile → Returning: ${userModel != null ? "UserModel loaded" : "null"}');
  return userModel;
}
...
static Future<bool> updateUser(UserModel userModel) async {
  ...
  try {
    print('🟡 Start updating user: ${userModel.id}');
    print('📦 Data to update: ${userModel.toJson()}');
  ...
  print('🔚 Finished updateUser → Result: $isUpdate');
  return isUpdate;
}
...
static Future<bool> updateMenu(ProductModel menu) async {
  try {
    print('🔧 Start updateMenu');

    if (menu.id == null || menu.id!.isEmpty) {
      print('❌ vendorID is null or empty');
      return false;
    }

    print('📄 Collection: ${CollectionName.vendorProducts}');
    print('🏷️ Vendor ID: ${menu.id}');
    print('🧾 JSON Data: ${menu.toJson()}');
  ...
  static Future<bool> deleteProductFromMenu(ProductModel menu) async {
  // static Future<bool> deleteProductFromMenu(MenuModel menu) async {
    try {
      if (menu.vendorID == null || menu.vendorID!.isEmpty) {
      // if (menu.productId == null || menu.productId!.isEmpty) {
        print("productId ID is null or empty");
        return false;
      }

      DocumentReference docRef =
          fireStore.collection(CollectionName.menu).doc(menu.vendorID);
          // fireStore.collection(CollectionName.menu).doc(menu.menuId);

      DocumentSnapshot docSnap = await docRef.get();

      if (!docSnap.exists) {
        print("Document not found: ${menu.vendorID}");
        // print("Document not found: ${menu.productId}");
        return false;
      }

      await docRef.delete();
      print("Deleted product with ID: ${menu.vendorID}");
      // print("Deleted product with ID: ${menu.productId}");
      return true;
    } catch (e) {
  ...
  static Future<List<ProductModel>> getMenuForUser() async {
  try {
    String vendorID = Constant.userModel!.vendorID!;
    print("Fetching menu for userModel: ${Constant.userModel!.id}");
    print("Fetching menu for vendorID: $vendorID");
    ...
    List<ProductModel> products = snapshot.docs.map((doc) {
      ...
      // اطبع الـ Raw Data قبل التحويل
      print("\n🔥 Raw Firebase data:");
      data.forEach((key, value) {
        print("$key: $value");
      });

      // تحويل للكائن وطباعة النتيجة
      ProductModel product = ProductModel.fromJson(data);
      print("✅ Converted ProductModel: ${product.toJson()}");

      return product;
    }).toList();

    print("\n🎯 Finished loading ${products.length} products.\n");

    return products;
  } catch (e, stack) {
    print("❌ Error getting menus for user: $e");
    print("📌 Stacktrace:\n$stack");
    return [];
  }
}
  ...
  static Future<ProductModel> firebaseCreateNewMenu(ProductModel menu) async {
  // static Future<MenuModel> firebaseCreateNewMenu(ProductModel menu) async {
  // static Future<MenuModel> firebaseCreateNewMenu(MenuModel menu) async {
    // Assign menuId to match the user/vendor ID
    DocumentReference documentReference =
        fireStore.collection(CollectionName.menu).doc();

    // Assign vendor ID as the menu ID field inside the document
    menu.vendorID = Constant.userModel!.id;
    // menu.menuId = Constant.userModel!.vendorID;

    // Optional: assign the user ID if needed separately
    // menu.id = Constant.userModel!.id;
    // menu.productId = Constant.userModel!.id;

    // Save the generated document ID (optional, if you want to track it)
    // menu.documentId = documentReference.id; // Only if MenuModel has this field

    // Save to Firestore
    await documentReference.set(menu.toJson());

    // Optional: update the existing menu if needed
    if (Constant.menuModel != null) {
      await FireStoreUtils.updateMenu(Constant.menuModel!);
      await FireStoreUtils.updateMenu(Constant.menuModel!);
    }
    ...
    await fireStore
      ...
      if (value.docs.isNotEmpty) {
        for (var element in value.docs) {
          ...
          Constant.createStory =
              subscriptionPlanModel.features?.createStory ?? false;
